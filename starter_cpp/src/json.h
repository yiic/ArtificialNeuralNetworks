...
//     Dohickey(JsonNode* ob) :
//       b(ob->getBool("b")),
//       i((int)ob->getInt("i")),
//       d(ob->getDouble("d")),
//       t(ob->get("t")),
//     {
//         JsonListIterator it(ob->get("l"));
//         while(it.remaining())
//             l.push_back(new Thingy(it.current());
//     }
//
//
...
//     {
//         JsonNode* ob = doc.newObj();
//         ob->add(&doc, "b", b);
//         ob->add(&doc, "i", i);
//         ob->add(&doc, "d", d);
//         ob->add(&doc, "t", t.marshal());
//         JsonNode* ll = doc.newList();
//         ob->add(ll);
...
public:
	...
	JsonNode* getIfExists(const char* szName) const;
	...
	JsonNode* get(const char* szName) const
	{
		JsonNode* pNode = getIfExists(szName);
		if(!pNode)
	...
	JsonNode* add(Json* pDoc, const char* szName, JsonNode* pNode);

	/// Adds a boolean field
	JsonNode* add(Json* pDoc, const char* szName, bool b);

	/// Adds an int field
	JsonNode* add(Json* pDoc, const char* szName, long long n);

	/// Adds an int field
	JsonNode* add(Json* pDoc, const char* szName, size_t n);

	/// Adds a double field
	JsonNode* add(Json* pDoc, const char* szName, double d);

	/// Adds a string field
	JsonNode* add(Json* pDoc, const char* szName, const char* str);

	/// Adds an item to a list node. Returns a pointer to the item passed in (pNode).
	JsonNode* add(Json* pDoc, JsonNode* pNode);

	/// Adds a boolean list item
	JsonNode* add(Json* pDoc, bool b);

	/// Adds an int list item
	JsonNode* add(Json* pDoc, long long n);

	/// Adds an int list item
	JsonNode* add(Json* pDoc, size_t n);

	/// Adds a double list item
	JsonNode* add(Json* pDoc, double d);

	/// Adds a string list item
	JsonNode* add(Json* pDoc, const char* str);
