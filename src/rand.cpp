...
double Rand::weibull(double gamma_val)
{
	if(gamma_val <= 0)
		throw Ex("invalid parameter");
	return pow(exponential(), (1.0 / gamma_val));
}
...
double Rand::beta(double alpha_val, double beta_val)
{
	if(alpha_val <= 0 || beta_val <= 0)
		throw Ex("invalid parameters");
	double r = gamma(alpha_val);
	return r / (r + gamma(beta_val));
}
